...
const EMPTY_ADMIN_MENU_ITEMS: AdminMenuItem[] = [];
...
export default function AdminOrders({ settingsOnly = false }: { settingsOnly?: boolean }) {
  ...
  const { data: rawMenuItems = EMPTY_ADMIN_MENU_ITEMS } = trpc.menu.list.useQuery();
  ...
  useEffect(() => {
    ...
    const normalizedHours = normalizeStoreHours(storeHoursData);

    setStoreHoursForm((currentHours) =>
      currentHours.openingTime === normalizedHours.openingTime &&
      currentHours.closingTime === normalizedHours.closingTime
        ? currentHours
        : normalizedHours,
    );
  }, [storeHoursData]);
  ...
  useEffect(() => {
    ...
    const nextSettings = availability.settings;
    const nextAcceptingOrders = Boolean(nextSettings.acceptingOrders);

    setOrderSettingsForm((currentSettings) =>
      currentSettings.acceptingOrders === nextSettings.acceptingOrders &&
      currentSettings.dailyOrderLimit === nextSettings.dailyOrderLimit
        ? currentSettings
        : nextSettings,
    );
    setAcceptingOrders((currentAcceptingOrders) =>
      currentAcceptingOrders === nextAcceptingOrders
        ? currentAcceptingOrders
        : nextAcceptingOrders,
    );
    ...
    <div className="admin-page min-h-screen w-full overflow-x-hidden bg-white">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex min-h-16 flex-col gap-3 py-3 md:flex-row md:items-center md:justify-between">
            ...
            <div className="grid w-full grid-cols-3 gap-2 md:flex md:w-auto md:flex-wrap md:items-center md:justify-end md:gap-3">
              <Link href="/dashboard" className="hidden sm:inline text-gray-700 hover:text-green-700 font-medium transition">
                Dashboard
              </Link>
      ...
      <section className="py-10 bg-gradient-to-r from-green-600 to-green-700">
        <div className="max-w-7xl mx-auto px-4">
          <h1 className="text-4xl font-bold text-white mb-2">{settingsOnly ? "Store Settings" : "Orders"}</h1>
          <p className="text-green-100 text-lg">
            {settingsOnly
              ? "Manage pickup hours, checkout status, and stock."
      ...
      <main className="mx-auto w-full max-w-7xl px-4 py-6 sm:px-6 sm:py-10 lg:px-8">
        ...
        {!settingsOnly && !isLoading && orders.length === 0 && (
