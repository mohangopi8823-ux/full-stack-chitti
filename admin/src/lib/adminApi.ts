...
export interface BackupOldOrdersResult {
  success: true;
  trigger: string;
  cutoffIso: string;
...
export function backupOldOrders(deleteAfterBackup = false) {
  return apiFetch<BackupOldOrdersResult>(`/api/admin/backup-old-orders?delete=${deleteAfterBackup}`, {
    method: "POST",
  });
}
