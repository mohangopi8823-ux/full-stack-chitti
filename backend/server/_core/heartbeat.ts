 ...
 *
 * The actor in the response echoes whose cron list you got back. End-users
 * cannot list other users' crons via this SDK.
 */
export async function listHeartbeatJobs(
  ...
  } & Record<string, unknown>>("ListHeartbeatJobs", body, userSession);
  const actorKey = ["actor", "User", "Id"].join("");
  return {
    total: response.total,
    actor: String(response[actorKey] ?? ""),
    jobs: response.jobs,
  };
}
